...
var formatMoney = function (num) {
      ...
      var unformatMoney = function(formattedMoney){
        var unformatted =  formattedMoney.toString().replace(/,/g, '');
...
function calculatePMT()  {
  ...
  //document.getElementById('amortization').style.display = "none";
    ...
    tax = parseFloat(tax);
    var payment = tax + (interest/100) / (1 - (Math.pow(1 + interest/100, -parseFloat(unformatMoney(termMonths))))) * (unformatMoney(price) - unformatMoney(downPay));
    lblPayment.innerHTML = formatMoney(payment);
    price = unformatMoney(price);
    downPay= unformatMoney(downPay);
    lblTotalPayment.innerHTML = formatMoney(unformatMoney(lblTotalPayment));
    ...
    lblTotalLoan.innerHTML = formatMoney(unformatMoney((payment - tax) * termMonths));
    lblTotalInterest.innerHTML = formatMoney((payment-tax) * termMonths - (price - downPay));
